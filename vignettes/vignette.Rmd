...
We firstly check the distribution of our missing values across all cell lines. Below is the plot of remaining celllines depending on cutoff for the maximum number of missing values per column (cellline).  
```{r}
missTab <- data.frame(
  NA_cutoff = 0,
...
A function to perform association test between target importance and cancer type or genomic features. If the features have only two levels, Student's t-test will be use. If the features have more than two levels, ANOVA test will be used. 
```{r}
...
Perform association test between protein dependence and cancer type or mutational background
```{r gdsc t-test}
...
pList$MAP2K2_NRAS
```
Boxplot showing the significant association between MAP2K2 protein dependence and NRAS mutation in GDSC cancer cell lines.
