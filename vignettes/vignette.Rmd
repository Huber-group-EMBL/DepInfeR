...
```{r}
ProcessTargetResults <- processTarget(targetMatrix, KdAsInput = TRUE , 
...
responseMatrix <- filter(drug_response_GDSC, `Drug Id` %in% targetsGDSC$drugID) %>%
    dplyr::select(Drug, `Drug Id`, `Cell line name` , `Cosmic sample Id`, 
                  `TCGA classification`, IC50, AUC, `Max conc`,RMSE, `Z score`) %>% 
...
```{r, fig.cap = "Plot of remaining celllines depending on cutoff for the maximum number of missing values per column (cellline)"}
missTab <- data.frame(NA_cutoff = character(0), 
                      remain_celllines = character(0), stringsAsFactors = FALSE)
...
plot(missTab, type = "l")
```
By looking at the missing value distribution, we choose cell lines with a maximum of 24 missing values per cell line (= 35%) and will input the remaining missing values by using the MissForest imputation method.
...
overlappedDrugs <- intersect(rownames(responseMatrix_scaled), 
...
```{r}
set.seed(333)
result <- runLASSOregression(TargetMatrix = targetInput, 
...
levels(mutation_GDSC$TCGA.classification) <- 
    c(levels(mutation_GDSC$TCGA.classification), "BRCAHer-")
...
mutation_GDSC$TCGA.classification <- 
    factor(mutation_GDSC$TCGA.classification,
           levels = c("ALL", "LAML", "DLBC", "BRCAHer-", "BRCAHer+"))
pheatmap(plotTab, 
         ...
         show_colnames = TRUE, main = "", fontsize = 9, 
...
colnames(cell_anno_final) <- str_replace_all(colnames(cell_anno_final),
                                             "_trans","_translocation")
```

A customized function to perform association test.
```{r}
diffImportance <- function(coefMat, Annotation) {
  #process genetic background table
    ...
    }) %>% dplyr::bind_rows() %>% dplyr::arrange(p) %>% 
...
plotTab <- t(scale(t(result$coefMat))) %>% data.frame() %>%
    ...
    group_by(target, Cancer_Type) %>% mutate(meanCoef = mean(coef)) %>%
    arrange(meanCoef) %>% ungroup() %>%
...
ggplot(plotTab, aes(x = target, y = coef, group=Cancer_Type)) +
    ...
    scale_color_manual(values= c("#BC3C29FF","#E18727FF",
                                 "#20854EFF","#0072B5FF","#7876B1FF"),
                      guide = guide_legend(override.aes = list(size = 3) )) +
    ggtitle("Protein dependence associated with cancer type") + 
    ylab("Protein dependence coefficient") + xlab("Protein") + theme_bw() + 
...
```{r gdsc_pval_vis, fig.height=6, fig.width=10, fig.cap="Significant associations between mutational background and protein dependence of GDSC cancer cell lines."}
colList2 <- c(`not significant` = "grey80", mutated = "#BC3C29FF", 
              unmutated = "#0072B5FF")
...
plotDiffBox <- function(pTab, coefMat, cellAnno, fdrCut = 0.05) {
    ...
    if (str_detect(geno, "trisomy12|gain|del")) {
        plotTab <- mutate(plotTab, 
                          mut = ifelse(mut %in% c("1",1), "present","absent")) %>%
...
```{r gdsc boxplot, fig.height=4.5, fig.width=5, fig.cap="Boxplot showing the significant association between MAP2K2 protein dependence and NRAS mutation in GDSC cancer cell lines."}
